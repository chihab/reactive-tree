...
const add = (children): number => {
  return children.reduce((sum, val) => sum + val, 0)
}

const sub = ([v1, v2]): number => {
  return v1 - v2;
}
...
test("subscribe to a node having nodes as children", done => {
  const leaf1: Leaf<number> = new Leaf(6);
  const leaf2: Leaf<number> = new Leaf(11);
  const node1: Node<number> = new Node<number>([leaf1, leaf2], add);
  const node2: Node<number> = new Node<number>([leaf1, leaf1], add);
  const root: Node<number> = new Node<number>([node1, node2], sub);
...
test("subscribe to a node having nodes as children", done => {
  ...
  const root: Node<number> = new Node([node1, node2], sub);
  root.output$.subscribe(v => {
    expect(v).toBe(5);
    done();
  });
...
test("grand parent gets notified after adding child to a node", done => {
  ...
  const root: Node<number> = new Node([node1, node2], sub); // 5
